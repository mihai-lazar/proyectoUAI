class Camera {
  ...
  void init() {
    ...
    hs = new HScrollbar(width - thirdPix - margin/2, height - margin, thirdPix, margin, 10);
    
    // Initialize Vertical Scrollbar
    vs = new VScrollbar(width - margin, height - thirdPix - margin*2, margin, thirdPix, 10);
  ...
  void on() {
    ...
    // Camera Position
    eyeX = boundary.x * 0.5;
    eyeY = 0.50 * boundary.y - zoom*3 * 0.50 * boundary.y;
  ...
  void reset() {
    ...
    //vs.newspos = vs.sposMax - (vs.sposMax - vs.sposMin) * (ZOOM_DEFAULT) / (ZOOM_MIN-ZOOM_MAX);
    vs.newspos = vs.sposMax - (vs.sposMax - vs.sposMin) * (ZOOM_DEFAULT) /  (ZOOM_MIN-ZOOM_MAX);
    drag.x_offset = 0;
    drag.y_offset = 0;
  ...
  void drawControls() {
    ...
    text("PRESIONA 'C' PARA RE CENTRAR LA CAMARA", 0, 0);
...
class HScrollbar {
  ...
  void display(int lineColor, int baseAlpha) {
    if (enable) {
      ...
      text("ROTACIÓN", xpos + swidth/2, ypos + sheight/1.2);
    }
  }
...
class VScrollbar {
  ...
  void display(int lineColor, int baseAlpha) {
    if (enable) {
      ...
      text("ZOOM", xpos + swidth/2, ypos + sheight/2);
    }
  }
