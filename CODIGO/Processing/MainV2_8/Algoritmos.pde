void mover_grua(){
  ...
  if(movimientos.size() > 0){
    ...
    if(movimientos.get(0).buscado && movimientos.get(0).container.a_ocupar){
      if(Grua.location.z < 260){
      ...
      }else if((Grua.location.y > movimientos.get(0).mover_a.y && estado_arduino.equals("Z+")) || estado_arduino.indexOf(estado) >= 0){
        Grua.location.y--;
        ...
        arduinoPort.write("X" + str(movimientos.get(0).container.pos_x) + "\n");  
        movimientos.get(0).container.location.y++;
      }else if((Grua.location.x > movimientos.get(0).mover_a.x && estado_arduino.equals("Z+")) || estado_arduino.indexOf(estado) >= 0){
        Grua.location.x--;
