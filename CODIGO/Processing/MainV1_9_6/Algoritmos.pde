  ...
  if(movimientos.size() > 0){
    ...
    if(movimientos.get(0).buscado){
      if(Grua.location.y > movimientos.get(0).mover_a.y)
        ...
        //mover(movimientos.get(0).container_de, new PVector (movimientos.get(0).container_a.pos_x, movimientos.get(0).container_a.pos_y, movimientos.get(0).container_a.pos_z));
        //movimientos.remove(0);
      }
    }
  }
