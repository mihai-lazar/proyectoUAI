...
void mover(Container container_a_mover, PVector lugar_donde_mover){
  ...
  for(int i = 0; i < 3; i++){
    if(Containers[int(lugar_donde_mover.x)][int(lugar_donde_mover.y)][i].a_ocupar == false && Containers[int(lugar_donde_mover.x)][int(lugar_donde_mover.y)][i].ocupado == false){
  ...
  if(espacio){
    ...
    Containers[int(lugar_donde_mover.x)][int(lugar_donde_mover.y)][int(lugar_donde_mover.z)] = container_a_mover;
