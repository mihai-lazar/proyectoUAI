...
for y in range (0, size_y):
    for x in range (0, size_x):
        for z in range (0, size_z):
            ...
            temp = []
            temp.append(0)
            temp.append(Button(window, text=' ', bg='white', padx=pad_x_container, pady=pad_y_container, command=(lambda x=y, y=size_y-1-x, z=size_z-1-z: action(x,y,z))))
            #Ingresamos la estructura a la matriz, en la posicion X,Y,Z correspondiente 
            btns[y][size_y-1-x][size_z-1-z] = temp
            btns[y][size_y-1-x][size_z-1-z][1].grid(row=x+z+offset_x, column=y+offset_y)
            btns[y][size_y-1-x][size_z-1-z][1].configure(text=str(y) + "," + str(size_y-1-x) + "," + str(size_z-1-z))
            cont_btn+=1
