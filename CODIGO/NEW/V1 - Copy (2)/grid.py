...
for x in range (0, size_x):
    for y in range (0, size_y):
        for z in range (0, size_z):
            ...
            btns[x][size_y-1-y][size_z-1-z][1].grid(row=y+z+offset_y + 2, column=x+offset_x)
            ...
            if(cont_btn % size_z == 0):
                ...
                labs[cont_lab].grid(row=y+z+offset_y + 3, column=x+offset_x)
                cont_lab += 1
        offset_y += size_z
    offset_y = 0
    offset_x += 1
...
for c in range (-1, 2):
    ...
    camn[c + 1].grid(row=(c + int(size_y/2))*(size_z + 1) - 1 + 2, column=size_x + offset_x)


cabezal = []
cabezal.append(0)
cabezal.append(Button(window, text="", bg="brown", state=DISABLED))
cabezal[1].place(x=btns[0][0][0][1].winfo_x()+pad_x_container, y=btns[0][0][0][1].winfo_y(), width=30, height=30)
cabezal.append(btns[0][0][0][1].winfo_x()+pad_x_container)
cabezal.append(btns[0][0][0][1].winfo_y())
