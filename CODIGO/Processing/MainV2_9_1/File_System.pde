...
void fileSelectedInputSeq(File selection) {
  if (selection == null) {
    ...
    if(trim(lines[0]).equals("###SEQ")){
      ...
      for (int i = 0 ; i < lines.length - 1; i++) {
        int[][] container_1 = new int[-1,-1,-1][-1,-1,-1];
        int[][] container_2 = new int[-1,-1,-1][-1,-1,-1];
        for (int e = 0 ; e < lines.length - 1; e++) {
          if(Seq[i][0][0] == Seq[e][0][0] && Seq[i][0][1] == Seq[e][0][1] && Seq[i][0][2] =! Seq[e][0][2]){
            if(container_1[0] == -1){
              container_1[0][1] = Seq[e][0][0]
              container_1[0][2] = Seq[e][0][1]
              container_1[0][3] = Seq[e][0][2]
              
              container_1[1][1] = Seq[e][1][0]
              container_1[1][2] = Seq[e][1][1]
              container_1[1][3] = Seq[e][1][2]
            }else if(container_1[0] == -1){
              container_2[0][1] = Seq[e][0][0]
              container_2[0][2] = Seq[e][0][1]
              container_2[0][3] = Seq[e][0][2]
              
              container_2[1][1] = Seq[e][1][0]
              container_2[1][2] = Seq[e][1][1]
              container_2[1][3] = Seq[e][1][2]          
            }             
            Seq[e][0][0] = -1;
            Seq[e][0][1] = -1;
            Seq[e][0][2] = -1;
        ...
        for(int i = 0; i < largo ; i++){
          for(int z = 2; z > 0 ; z--){
            for(int e = 0 ; e < largo ; e++){
              if(seq_Container.get(i).container.pos_x == seq_Container.get(e).container.pos_x && seq_Container.get(i).container.pos_y == seq_Container.get(e).container.pos_y && seq_Container.get(i).container.pos_z == z && seq_Container.get(i).container.pos_z != seq_Container.get(e).container.pos_z){
                seq_Containe_newOrder.add(new Seq_Container(seq_Container.get(e).mover_a, seq_Container.get(e).container));
              }           
            }
      }
