...
void mouseClicked() { if (initialized) {
  ...
  if (cam.chunkField.closestFound && cam.enableChunks && !cam.hoverGUI()) {
    ...
    int temp_x = cam.chunkField.closest.pos_x;
    int temp_y = cam.chunkField.closest.pos_y;
    if (mouseButton == LEFT) {
      if(temp_x < 3){
        for(int z=0; z<3; z++){
        if(!Containers[temp_x][temp_y][z].ocupado){
          ...
          if(!selected){                       
            ...
            if(selecion_x != temp_x || selecion_y != temp_y){
              if(selecion_z < 2){
                for(int i = 2; i > selecion_z; i--){
                  if(Containers[selecion_x][selecion_y][i].ocupado == true)
                    //mover(Containers[selecion_x][selecion_y][i], buscar_libre_cercano(Containers[selecion_x][selecion_y][i]));
                    buscar_libre_cercano(Containers[selecion_x][selecion_y][i]);
                }
              }
              ...
              Containers[temp_x][temp_y][z] = new Container(temp_x, temp_y, z, location, Containers[selecion_x][selecion_y][selecion_z].fill, 1);
              Containers[temp_x][temp_y][z].ocupado = true;
